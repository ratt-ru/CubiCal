...
import math
import cyphase_only as cyphase
import argparse
import sys
import cPickle
...
def compute_js(obser_arr, model_arr, gains, t_int=1, f_int=1):
    ...
    n_dir, n_timint, n_freint, n_ant, n_cor, n_cor = gains.shape

    float_type = np.float64 if obser_arr.dtype is np.complex128 else np.float32

    gh = gains.transpose(0,1,2,3,5,4).conj()

    jh = np.zeros_like(model_arr)

    cyphase.cycompute_jh(model_arr, gains, jh, t_int, f_int)
...
def compute_update(model_arr, obser_arr, jhjinv, gains, t_int=1, f_int=1):
    """
    ...
    jhr = compute_js(obser_arr, model_arr, gains, t_int, f_int)
...
def solve_gains(obser_arr, model_arr, flags_arr, min_delta_g=1e-6, maxiter=30,
    ...
    if verbose > 0:
        ...
        logopts = (label, init_chi, num_valid_intervals, n_int, mineqs, maxeqs, anteqs,
                                                                        n_ant,  fstats)

        print>> log, ("{} Initial chisq = {:.4}, {}/{} valid intervals (min {}/max {} eqs per int)," 
                      " {}/{} valid antennas. Flags are: {}").format(*logopts)

    min_quorum = 0.99
    warned_null_gain = warned_boom_gain = False

    jhj = np.zeros(gain_shape, dtype=float_type)
    cyphase.cycompute_jhj(model_arr, jhj, t_int, f_int)
    cyphase.cycompute_jhjinv(jhj)
    ...
    while n_cnvgd/n_sols < min_quorum and n_stall/n_int < min_quorum and iters < maxiter:
        ...
        if iters % 2 == 0:
            gains -= 0.5*compute_update(model_arr, obser_arr, gains, t_int, f_int)
        else:
        ...
        if (iters % chi_interval) == 0:
            ...
            if verbose > 1:
                ...
                logopts = (label, iters, mean_chi, delta_chi, diff_g.max(), n_cnvgd/n_sols, 
                                                                            n_stall/n_int  )

                print>> log, ("{} iteration {} chi-sq is {:.4} delta {:.4}, max gain update {:.4}, "
                              "converged {:.2%}, stalled {:.2%}").format(*logopts)

    logopts = (label, iters, n_cnvgd/n_sols, n_stall/n_int, init_chi, mean_chi)

    print>>log, ("{}: {} iterations done. Converged {:.2%}, stalled {:.2%}. "
                "Chisq {:.4} -> {:.4}").format(*logopts)
