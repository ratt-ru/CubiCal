...
class PhaseSlopeGains(ParameterisedGains):
    ...
    def __init__(self, label, data_arr, ndir, nmod, chunk_ts, chunk_fs, chunk_label, options):
        ...
        if options["slope-type"]=="inverse":
            self.chunk_fs = _normalize(1/chunk_fs, self.ftype)
        elif options["slope-type"]=="inverse-squared":
            self.chunk_fs = _normalize(1/(chunk_fs**2), self.ftype)
        else:
