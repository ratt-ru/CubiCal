...
def cycompute_jhj(float3264 [:,:,:,:,:,:,:] tmp_jhj,
                  ...
                  int f_int):
    """
    Given the intermediary J\ :sup:`H`\J, times and channel frequencies, computes the diagonal 
    entries of J\ :sup:`H`\J. J\ :sup:`H`\J is computed over intervals. This is an approximation of 
    the Hessian. In the phase-only frequency slope case, this approximation does not depend on the 
    ...
    Args:
        ...
        ts (np.float32 or np.float64):
            Typed memoryview of times with dimension (t).
...
def cycompute_jhr(float3264 [:,:,:,:,:,:] tmp_jhr,
                  ...
                  int f_int):
    """
    Given the intermediary J\ :sup:`H`\R, times and channel frequencies, computes J\ :sup:`H`\R. 
            ...
            for f in xrange(n_fre):
                ...
                for aa in xrange(n_ant):
                    for c in xrange(2):
                        ...
                        jhr[d,rr,rc,aa,2,c,c] = jhr[d,rr,rc,aa,2,c,c] +       tmp_jhr[d,t,f,aa,c,c]
...
def cyconstruct_gains(float3264 [:,:,:,:,:,:,:] param,
    ...
    Args:
        ...
        g (np.complex64 or np.complex128):
            Typed memoryview of inverse gain array with dimensions (d, ti, fi, a, c, c).
        ts (np.float32 or np.float64):
            Typed memoryview of channel times with dimension (t).
        fs (np.float32 or np.float64):
            Typed memoryview of channel frequencies with dimension (f).
        t_int (int):
            Number of time slots per solution interval.
