...
def cycompute_jhj(float3264 [:,:,:,:,:,:,:] tmp_jhj,
                  ...
                  int f_int):
    """
    Given the intermediary J\ :sup:`H`\J and times, computes the diagonal entries of 
    J\ :sup:`H`\J. J\ :sup:`H`\J is computed over intervals. This is an approximation of the 
    Hessian. In the phase-only time slope case, this approximation does not depend on the 
    ...
    for d in xrange(n_dir):
        for t in xrange(n_tim):
            ...
            for f in xrange(n_fre):
                ...
                for aa in xrange(n_ant):
                    for c in xrange(2):
                        jhr[d,rr,rc,aa,0,c,c] = jhr[d,rr,rc,aa,0,c,c] + ts[t]*tmp_jhr[d,t,f,aa,c,c]
